...
def compute_pv_production(df_weather: pd.DataFrame) -> pd.DataFrame:
    if not PVLIB_AVAILABLE:
        ...
        # GHI(W/m2)/1000 = kW/m2 ; × surface(m2) = kW ; × eff × PR ; /1000 = MW
        # (correction : /1000 et non /1e6 - sinon production 1000x trop faible)
        df["P_AC_MW"] = (df["GHI"] / 1000) * PARK["module_efficiency"] * PARK["pr_p50"] * surface_m2 / 1000
